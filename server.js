const Discord = require("discord.js");
const client = new Discord.Client();

// =========================
// 起動確認
// =========================
client.on("ready", () => {
  console.log("✅ Bot logged in as " + client.user.tag);
  client.user.setActivity("TRPG Dice", { type: "PLAYING" });
});

// =========================
// メッセージ受信
// =========================
client.on("message", message => {
  if (message.author.bot) return;

  const content = message.content.trim();

  // =========================
  // 🎲 通常ダイス（1d6 / 1d6+1d4 / 1d100-5 等）
  // =========================
  if (/^\d+d\d+/i.test(content)) {
    const result = rollDiceExpression(content);
    if (result) {
      message.channel.send(`🎲 ${content} → ${result}`);
    }
    return;
  }

  // =========================
  // 🗡 ダメージ表
  // =========================
  if (content.startsWith("ダメージ")) {
    const weapon = content.replace(/^ダメージ\s*/,"");

    const damageTable = {
      "素手":"1D6-2","パンチ":"1D6-2","キック":"1D6-2","頭突き":"1D6-2",
      "体当たり":"1D6-2","投げ":"1D6-2","引き倒し":"1D6-2",
      "絞め":"1D6","首絞め":"1D6",
      "ナックル":"1D6","スパイク靴":"1D6",
      "ポケットナイフ":"1D6-2","小型ナイフ":"1D6-2","メス":"1D6-2",
      "ナイフ":"1D6","軍用ナイフ":"2D6-2","包丁":"2D6-2",
      "木刀":"1D6","フルーレ":"2D6-2","模造刀":"2D6-2","仕込み杖":"2D6-2",
      "サーベル":"2D6","小太刀":"2D6","日本刀":"3D6-2","脇差":"3D6-2",
      "大太刀":"3D6","古刀":"3D6","長脇差":"3D6",
      "草刈り鎌":"1D6+2","山刀":"1D6+2","ナタ":"1D6+2",
      "手斧":"2D6","大ナタ":"2D6","木こり斧":"3D6",
      "杖":"1D6-2","ステッキ":"1D6-2","警棒":"1D6",
      "ブラックジャック":"1D6+2","棍棒":"1D6+2","金槌":"1D6+2",
      "鉄杖":"1D6+2","バール":"2D6-2","大型ハンマー":"2D6-2",
      "金属警棒":"2D6-2","つるはし":"2D6+2","破壊槌":"2D6+2",
      "短槍":"2D6-2","銛":"2D6-2","銃剣":"2D6",
      "長槍":"3D6-2","薙刀":"3D6-2","三又槍":"3D6-2",
      "ハサミ":"1D6-2","フォーク":"1D6-2","アイスピック":"1D6-2",
      "陶器":"1D6-2","ガラス製品":"1D6-2","瓶":"1D6-2",
      "石":"1D6","レンガ":"1D6","植木鉢":"1D6",
      "フライパン":"1D6","アイロン":"1D6",
      "モップ":"1D6","椅子":"1D6","机":"1D6","看板":"1D6",
      "鉄パイプ":"2D6-2",
      "投石":"1D6","和弓":"3D6-2","ボウガン":"3D6",
      "小口径拳銃":"2D6","中口径拳銃":"3D6",
      "大口径拳銃":"3D6+2"
    };

    if (damageTable[weapon]) {
      message.channel.send(`🗡 ${weapon} / ${damageTable[weapon]}`);
    } else {
      message.channel.send("❓ その武器は登録されていません");
    }
    return;
  }

  // =========================
  // 🧠 短期狂気表
  // =========================
  if (["一時的狂気","短期的狂気","短期狂気"].includes(content)) {
    const table = [
      "気絶あるいは金切り声",
      "パニック状態で逃走",
      "感情の噴出（大笑い・大泣き）",
      "意味不明な多弁症",
      "極度の恐怖症",
      "殺人癖／自殺癖",
      "幻覚・妄想",
      "反響動作・反響言語",
      "異様な物を食べたがる",
      "昏迷／緊張症"
    ];
    const roll = rollDice(1,10);
    message.channel.send(`🧠 短期的狂気 (${roll})：${table[roll-1]}`);
    return;
  }

  // =========================
  // 🧠 長期狂気表
  // =========================
  if (["長期的狂気","長期狂気","不定の狂気","不定狂気"].includes(content)) {
    const table = [
      "健忘症／昏迷",
      "激しい恐怖症",
      "幻覚",
      "奇妙な性的嗜好",
      "フェティッシュ",
      "制御不能のチック",
      "心因性障害",
      "短時間の心因反応",
      "偏執症",
      "強迫観念行動"
    ];
    const roll = rollDice(1,10);
    message.channel.send(`🧠 長期的狂気 (${roll})：${table[roll-1]}`);
    return;
  }
});

// =========================
// 🎲 ダイス処理関数
// =========================
function rollDice(count, sides) {
  let total = 0;
  for (let i=0;i<count;i++) {
    total += Math.floor(Math.random()*sides)+1;
  }
  return total;
}

function rollDiceExpression(expr) {
  let total = 0;
  const parts = expr.match(/[+-]?[^+-]+/g);
  for (const p of parts) {
    if (p.includes("d")) {
      const [c,s] = p.replace("+","").split("d").map(Number);
      total += rollDice(c,s);
    } else {
      total += Number(p);
    }
  }
  return total;
}

// =========================
// 🔑 トークン
// =========================
client.login("★ここにBotトークン★");
